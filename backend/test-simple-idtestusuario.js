...
console.log('🚀 Solución simple con IdTestUsuario...\n');

async function testSimpleIdTestUsuario() {
  try {
    const pool = await poolPromise;
    
    console.log('📊 Probando solución simple con IdTestUsuario...');
    const startTime1 = Date.now();
    
    // Solución simple: Usar IdTestUsuario como criterio de unicidad
    const empresasQuery = `
SELECT TOP 20
  e.IdEmpresa,
  e.Nombre AS empresa,
  ...
  tu.Test,
  tu.IdTestUsuario,
  -- Calcular TestRank y TestCount por empresa usando una subconsulta
...
`;
    ...
    console.log(`   ✅ Solución simple con IdTestUsuario: ${endTime1 - startTime1}ms (${result1.recordset.length} registros)`);
...
`;
    ...
    console.log('\n📈 Resumen final:');
    console.log(`   • Solución simple con IdTestUsuario: ${endTime1 - startTime1}ms`);
    console.log(`   • Conteo total: ${endTime2 - startTime2}ms`);
    ...
    console.error('❌ Error en solución simple con IdTestUsuario:', error.message);
  }
}

// Ejecutar prueba
testSimpleIdTestUsuario().then(() => {
  console.log('\n✨ Solución simple con IdTestUsuario completada!');
