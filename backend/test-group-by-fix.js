...
console.log('🚀 Prueba final con GROUP BY: Verificando eliminación completa de duplicados...\n');

async function testGroupByFix() {
  try {
    const pool = await poolPromise;
    
    console.log('📊 Probando consulta con GROUP BY...');
    const startTime1 = Date.now();
    
    // Simular la consulta real con GROUP BY
    const empresasQuery = `
...
WITH Base AS (
  ...
  GROUP BY 
      e.IdEmpresa, e.Nombre, ei.IdUsuario, tu.Test, 
      ei.TotalEmpleados, ei.IdDepartamento, ei.IdLocalidad, 
      ei.IdSectorActividad, ei.IdVentas
),
...
`;
    ...
    console.log(`   ✅ Consulta con GROUP BY: ${endTime1 - startTime1}ms (${result1.recordset.length} registros)`);
    ...
    if (empresas.length === uniqueCombinations.length) {
      ...
      console.log(`\n   🔍 Duplicados encontrados (primeros 10):`);
...
WITH Base AS (
  ...
  JOIN dbo.Empresa e
    ON e.IdEmpresa = ei.IdEmpresa
  WHERE tu.Finalizado = 1
  GROUP BY e.IdEmpresa, e.Nombre, tu.Test
),
Ranked AS (
  SELECT
      u.*,
...
`;
    ...
    console.log('\n📈 Resumen final:');
    console.log(`   • Consulta con GROUP BY: ${endTime1 - startTime1}ms`);
    console.log(`   • Conteo total: ${endTime2 - startTime2}ms`);
    ...
    if (empresas.length === uniqueCombinations.length) {
      ...
      console.log('   ✅ Rendimiento optimizado con GROUP BY');
    } else {
    ...
    console.error('❌ Error en prueba con GROUP BY:', error.message);
  }
}

// Ejecutar prueba
testGroupByFix().then(() => {
  console.log('\n✨ Prueba con GROUP BY completada!');
